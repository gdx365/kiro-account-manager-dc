...
function MCPServerCard({ name, config, onToggle, onEdit, onDelete }: MCPServerCardProps) {
      ...
      <div className='flex items-start justify-between'>
        ...
        <div className='flex items-center gap-2 ml-4'>
          <button
            ...
            className={`cursor-pointer relative w-11 h-6 rounded-full transition-colors ${isDisabled ? colors.toggleOff : colors.toggleOn}`}
          >
            <div className={`absolute top-1 w-4 h-4 rounded-full transition-transform ${isDisabled ? 'left-1' : 'left-6'} ${colors.toggleThumb}`} />
          </button>
